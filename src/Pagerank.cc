...
void main(int argc, char* argv[]) {
  ...
  unsigned long compute_start_time, compute_end_time, build_start_time, build_end_time;
  ...
  MPI_Barrier(MPI_COMM_WORLD);
  if(rank == 0) {std::cout << "build start time: " << getTimeInMilliseconds(start) << std::endl;}
  ...
  gettimeofday(&end, 0);
  MPI_Barrier(MPI_COMM_WORLD);
  double build_time = (end.tv_sec-start.tv_sec)*1e3+(end.tv_usec-start.tv_usec)*1e-3;
  if(rank == 0) {std::cout << "build_end_time: " << getTimeInMilliseconds(end) << std::endl;}

  MPI_Barrier(MPI_COMM_WORLD);
  gettimeofday(&start, 0);
  MPI_Barrier(MPI_COMM_WORLD);
  if(rank == 0) {std::cout << "compute_start_time: " << getTimeInMilliseconds(start) << std::endl;}
  ...
  gettimeofday(&end, 0);
  MPI_Barrier(MPI_COMM_WORLD);
  double pr_time = (end.tv_sec-start.tv_sec)*1e3+(end.tv_usec-start.tv_usec)*1e-3;
  if(rank == 0) {std::cout << "compute_end_time: " << getTimeInMilliseconds(end) << std::endl;}
  ...
  double hdfs_time = (end.tv_sec-start.tv_sec)*1e3+(end.tv_usec-start.tv_usec)*1e-3;
